...
import { Send, MoreVertical, Phone, Video, ArrowLeft, MessageCircle, Smile, Image as ImageIcon, X, Reply as ReplyIcon, Edit2 } from 'lucide-react';
...
export const ChatWindow = ({
    ...
    const performSubmit = async (content: string, imageFile: File | null) => {
        ...
        try {
            if (editingMessage) {
                if (onEditMessage) {
                    await onEditMessage(editingMessage.id, content.trim());
                ...
                onSendMessage(content.trim(), imageUrl, replyingTo?.id);
                setNewMessage('');
                removeImage();
                setReplyingTo(null);
            }
    ...
    const handleForward = () => {
        if (contextMenu.message) {
            if (onForwardMessage) {
                ...
                navigator.clipboard.writeText(contextMenu.message.content);
                alert(t('chat.forward_copy_hint', 'Message copied to clipboard'));
            }
        }
        closeContextMenu();
    };
    ...
    return (
        <>
                    ...
                    <div className="bg-surface/60 backdrop-blur-3xl px-6 py-4 rounded-[32px] border border-outline-variant/20 flex items-center justify-between shadow-2xl shadow-primary/5">
                        <div className="flex items-center gap-4">
                            {onBack && (
                                <button
                                    onClick={onBack}
                                    className="p-2 -ml-2 hover:bg-surface-container rounded-full transition-all group"
                                    title={t('common.back')}
                                >
                            ...
                            <div>
                                <h2 className="font-black text-on-surface uppercase italic tracking-tight leading-none mb-1 text-sm md:text-base truncate max-w-[120px] md:max-w-none flex items-center gap-2">
                                    ...
                                    <UserBadge username={otherParticipant.username} isVerified={otherParticipant.is_verified} />
                                </h2>
                    ...
                    <div className="max-w-4xl mx-auto relative">
                        ...
                        >
                            ...
                            <button
                                ...
                                title={t('post.add_image')}
                                disabled={!!editingMessage} // Disable image upload when editing text
                            >
            ...
            {/* Context Menu - Moved outside to escape transform context */}
            <AnimatePresence>
