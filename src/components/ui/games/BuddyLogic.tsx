...
export const BuddyLogic: React.FC = () => {
    ...
    return (
        <div className="flex flex-col items-center gap-8 lg:gap-10 w-full max-w-sm lg:max-w-md mx-auto p-4 select-none accelerate">
            ...
            <div className={`touch-none relative w-full bg-gradient-to-br from-surface-container-low/40 to-transparent rounded-[3rem] lg:rounded-[4rem] border border-white/10 ${reduceEffects ? '' : 'backdrop-blur-3xl'} p-6 lg:p-8 flex flex-col items-center overflow-hidden shadow-[0_40px_80px_-20px_rgba(0,0,0,0.6)] accelerate`}>
                <AnimatePresence mode="wait">
                    {gameState === 'start' ? (
                        ...
                        <motion.div
                            ...
                            className="flex flex-col items-center justify-center text-center p-8 bg-surface-container-highest/60 rounded-[2.5rem] border border-white/15 backdrop-blur-2xl shadow-[0_0_100px_rgba(0,0,0,0.5)] relative overflow-hidden w-full max-w-[280px]"
                        >
                            <div className="absolute inset-0 bg-gradient-to-br from-error/20 via-transparent to-transparent pointer-events-none" />
                            <div className="w-12 h-12 rounded-[1.5rem] bg-error/20 flex items-center justify-center mb-4 shadow-xl shadow-error/10">
                                <Brain className="w-6 h-6 text-error" />
                            </div>
                            <h3 className="text-xl font-black text-error mb-2 uppercase italic tracking-tighter">{t('game.logic.fail')}</h3>
                        ...
                        >
                            ...
                            <div className="flex flex-col items-center gap-6">
                                ...
                                <div className="flex items-center justify-center gap-2 lg:gap-4 w-full flex-wrap">
                                    {sequence?.items.map((item, i) => (
                                        <div key={i} className="flex items-center gap-2 lg:gap-4 group">
                                            ...
                                            {i < sequence.items.length - 1 && (
                                                <motion.div
                                                    animate={{ x: [0, 5, 0], opacity: [0.3, 1, 0.3] }}
                                                    transition={{ duration: 1.5, repeat: Infinity, delay: i * 0.2 }}
                                                    className="w-2 sm:w-4 lg:w-6 h-[2px] bg-gradient-to-r from-primary/50 to-transparent rounded-full opacity-50"
                                                />
                                            )}
                                        </div>
                                    ))}
                                    <div className="w-2 sm:w-4 lg:w-6 h-[2px] bg-gradient-to-r from-primary/50 to-transparent rounded-full opacity-30" />
                                    <motion.div
                                        animate={{
                            ...
                            <div className="flex flex-col gap-4">
                                ...
                                <div className="grid grid-cols-2 gap-4 w-full relative z-10 pb-2">
                                    {sequence?.options.map((opt, i) => {
                                        ...
                                        return (
                                            <motion.button
                                                ...
                                                disabled={!!feedback}
                                                className={`
                                                    h-16 lg:h-20 rounded-[2rem] flex items-center justify-center text-xl lg:text-2xl font-black transition-all duration-300 border relative overflow-hidden
                                                    ${buttonStyle}
                                                `}
                                            >
                                                <div className="absolute inset-0 bg-gradient-to-tr from-white/5 to-transparent pointer-events-none" />
